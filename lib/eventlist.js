...
(function (root, name) {
  ...
  var vendorPrefixes = [
      ...
      vendorPrefixesLength = vendorPrefixes.length,
      _d = document,
   ...
   * @param {_event} String event name to check
   * @return {Object} Object consists of validity and sanitized eventName
   */
  function isObservable(_event) {
    var i,
        valid = false,
        vendor,
        cap = _event.charAt(0),
        Cap = cap.toUpperCase(),
        eventName = _event.substr(1);

    for (i = 0; i < vendorPrefixesLength; i++) {
      vendor = vendorPrefixes[i];
      if (("on" + vendor + _event) in root) {
        valid = new root.Boolean(true);
        valid.eventName = ((vendor) ? vendor + Cap : cap) + eventName;
        break;
      }
    }
  ...
  EventList.prototype.listen = function () {
    ...
    if (!_self.hasOwnProperty(_event)) {
      ...
      setProperty(_self[_event], "__event__", valid.eventName);
